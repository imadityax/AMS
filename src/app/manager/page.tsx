...
export default async function ManagerDashboard() {
  ...
  const employees = await prisma.user.findMany({
    ...
    include: {
      attendances: {  // ✅ Changed from 'attendance' to 'attendances'
        ...
        orderBy: {
          checkIn: 'desc'  // ✅ Changed from 'punchIn' to 'checkIn'
        },
        take: 1
      },
      tasks: {
  ...
  const presentToday = employees.filter(emp =>
    emp.attendances.length > 0 && emp.attendances[0].checkIn // ✅ Changed field names
  ).length;
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {employees.map((employee) => {
                const todayAttendance = employee.attendances[0]; // ✅ Changed from 'attendance' to 'attendances'
                const hoursWorked = todayAttendance?.checkOut // ✅ Changed from 'punchOut' to 'checkOut'
                  ? Math.round((new Date(todayAttendance.checkOut).getTime() -
                    new Date(todayAttendance.checkIn).getTime()) / (1000 * 60 * 60) * 10) / 10 // ✅ Changed field names
                  : todayAttendance?.checkIn // ✅ Changed from 'punchIn' to 'checkIn'
